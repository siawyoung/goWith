...
- (void)setViewItems {
	...
	_usernameView = [[BlurView alloc] initWithFrame:CGRectMake(60, 245, 250, 50)];
	_passwordView = [[BlurView alloc] initWithFrame:CGRectMake(60, 300, 250, 50)];

	_sendButtonView = [[UIView alloc] initWithFrame:CGRectMake(60, 370, 250, 50)];
	...
	[sendButton addTarget:self action:@selector(myAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)myAction {
	[[Client sharedInstance] signInToFirebaseWithEmail:usernameTf.text andPassword:passwordTf.text completion: ^(NSError *error) {
	    if (error) {
        }
	}];
}
